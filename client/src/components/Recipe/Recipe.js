...
const Recipe = () => {
  ...
  const { card, isLoading, isRecomendLoading, recommend } = useSelector(
    (state) => state.cards
  );
  // console.log(card)
  ...
  useEffect(() => {
    if (card) {
      dispatch(
        getRecommendSearch({
          Keywords: card?.RecipeIngredientParts.join(","),
          category: card?.RecipeCategory,
        })
      );
    }
  }, [dispatch, card]);
  ...
  const sentenceSplitting = (sentence) => {
    // Combine all elements of the array into a single string
    const combinedString = sentence.join(" ");

    // Split the combined string by full stops
    const splitSentences = combinedString.split(/(?<=\.) /);

    return splitSentences;
  };

  let recommendedCards = recommend.filter(({ _id }) => _id !== card._id);
  recommendedCards = recommendedCards.slice(
    0,
    Math.min(4, recommendedCards.length)
  );
        ...
        <div className="main-content">
            ...
            <div className="two-container">
              <div className="two-container-content">
                ...
                <div className="two-container-content-header">
                  <h1 style={{ color: "black" }} className="header__header">
                    {card?.Name}
                  </h1>
                  <div className="header__rating">
                    <div className="header__rating">
                      <Rating
                        name="half-rating"
                        value={card?.AggregatedRating}
                        precision={0.5}
                        readOnly
                      />
                      &nbsp;&nbsp;
                      <span className="rating-count">
                        {card?.CommentsCount} ratings
                      </span>
                    </div>
                  </div>
                  <div className="header__text">{card?.Description}</div>
                  {/* <div className="tags">  
                  {
            ...
            <div className="nutrients-info">
              <div className="nutrients">
                <ul className="ul-nutrients">
                  <li className="li-nutrients">
                    Carbohydrates : {card?.CarbohydrateContent}g
                  </li>
                  <li className="li-nutrients">
                    Sugar Content : {card?.SugarContent}g
                  </li>
                  <li className="li-nutrients">
                    Proteins : {card?.ProteinContent}g
                  </li>
                  <li className="li-nutrients">
                    Cholestrol Content : {card?.CholesterolContent}mg
                  </li>
                  <li className="li-nutrients">Fats : {card?.FatContent}g</li>
                  <li className="li-nutrients">
                    Fibre Content : {card?.FiberContent}g
                  </li>
                  <li className="li-nutrients">
                    Calories : {card?.Calories}cal
                  </li>
                  <li className="li-nutrients">
                    Sodium Content : {card?.SodiumContent}mg
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div className="steps">
            ...
            <div className="steps-box">
              <ol>
                {card?.RecipeInstructions &&
                  sentenceSplitting(card?.RecipeInstructions).map((rec) => (
                    <li key={rec}>{rec}</li>
                  ))}
              </ol>
            </div>
